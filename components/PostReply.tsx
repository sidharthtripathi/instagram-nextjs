...
export function PostReply({commentId,formEnabled} : {commentId : string,formEnabled : boolean}){
      ...
      <div>
        <form >
          <fieldset disabled = {formDisabled} className="flex gap-2">
            <Input type = 'text' placeholder="enter your reply" onChange={(e)=>{setComment(e.target.value)}} value={comment}/>
            <Button size={"sm"}
            disabled = {!comment}
            onClick={(e)=>{
                e.preventDefault()
